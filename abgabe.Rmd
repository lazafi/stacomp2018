...
subtitle: "Abgabe"
author: "Attila Lázár (Matr. Nr. 0026842) TODO:Mathias"
...
We use $mutate$ from $dplyr$ package to convert `rank_date`. This conversion is meaningfull because, as we can see later in the plots, the plotting library can format the dates appropriately. Also if we could analyse the data as timeseries.
...
As an simple metrik we calculated the median fifa-rank for every country. We plotted the first 20.
...
To merge the `FIFA Ranking` with the `Countries` dataset we grouped the `FIFA Ranking` by country_full. Additionaly we generated a `key` variable for merging with `Countries`.
